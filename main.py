...
class Blockchain:
    ...
    def is_chain_valid(self, chain):
        ...
        while block_index < len(chain):
            ...
            proof = block['proof']
            hash_operation = hashlib.sha256(str(proof ** 2 - previous_proof ** 1).encode()).hexdigest()
            if hash_operation[:4] != '0000':
                return False
...
def update_sell():
    ...
    my_hash = blockchain.hash(block)
    response = {'message': 'Congratulation, you just mined a block',
                'Total_sell': blockchain.total_bill,
                'index': block['index'],
                'timestamp': block['timestamp'],
                'proof': block['proof'],
                'previous_hash': block['previous_hash'],
